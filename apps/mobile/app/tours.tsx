...
import { Mountain, Navigation, Activity, Wind, Flag, Search,
  ...
  AlertTriangle, Check, BookOpen, ChevronLeft } from 'lucide-react-native';
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      {filtered.length===0 ? (
        <View style={styles.empty}>
          ...
          <Text style={styles.emptySub}>{search?`Keine Touren für "${search}" gefunden`:'Starte deine erste Tour'}</Text>
        </View>
      ) : null}
            ...
            <TouchableOpacity key={tour.id} style={styles.card}
              ...
              <View style={[styles.cardHero,{backgroundColor:heroColor}]}>
                <View style={styles.cardHeroLeft}>
                  <ActIcon size={15} color="rgba(255,255,255,0.8)" strokeWidth={2}/>
      ...
      {filtered.length > 0 && (
        <Text style={styles.pageInfo}>
          {page*PAGE_SIZE+1}–{Math.min((page+1)*PAGE_SIZE,filtered.length)} von {filtered.length} Touren
        </Text>
      )}
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  pagination:{flexDirection:'row',alignItems:'center',justifyContent:'center',gap:6,paddingVertical:16,paddingHorizontal:16},
