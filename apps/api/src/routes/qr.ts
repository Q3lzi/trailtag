...
const vehicle = await prisma.vehicle.findUnique({
  ...
  // PLANNED tours → always green (not started yet)
  if (!activeTour || isStale || activeTour.status === 'PLANNED') {
    return res.send(renderGreen(vehicle))
  }

  // ACTIVE but overdue → show alarm (cron bridge in case cron hasn't run yet)
  const etaMs = activeTour.eta ? new Date(activeTour.eta).getTime() : null
  const isOverdue = etaMs !== null && etaMs < Date.now()

  if (activeTour.status === 'ACTIVE' && !isOverdue) {
...
function renderGreen(vehicle: any, tour?: any) {
  const body = tour ? `
    ...
    <div class="card" style="text-align:center;padding:24px;color:#747871">
      <div style="font-size:32px;margin-bottom:8px">🔒</div>
      <div style="font-weight:700;color:#434841;margin-bottom:4px">Standort geschützt</div>
      <div style="font-size:13px">GPS-Standort wird aus Datenschutzgründen nur im Notfall angezeigt.</div>
    </div>
